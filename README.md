...
- **Chinese input (Rime)** — Embedded librime session inside the IBus engine.
  Shift toggles between EN/voice and Chinese (双拼) mode. Rime runs in-process;
  no second parallel IBus engine needed. Optional: gracefully disabled if
  librime is not installed.
- **Fedora / systemd launcher** — Plain bash launcher as an alternative to the
  Nix shell, for non-NixOS distros.
...
```
  ...
  Microphone ──▶ PyAudio ──▶ WebRTC VAD ──▶ Pre-Buffer (600ms)       │
                        ...
                        │  Rime session  ──▶ preedit candidates       │
                        │                    (Shift to toggle ZH/EN)  │
...
Edit the `<exec>` path in the XML to match your setup:
```bash
nano ~/.local/share/ibus/component/voice-typing-ibus.xml
```

**NixOS** — point to the generated wrapper script:
```xml
<exec>/home/your_username/voice-typing-linux/ibus-engine-voice-typing</exec>
```

**Other distros (Fedora, Ubuntu, Arch…)** — point directly to the engine and
prefix with your venv's Python:
```xml
<exec>/home/your_username/voice-typing-linux/.venv/bin/python /home/your_username/voice-typing-linux/ibus_voice_engine.py</exec>
```

Replace `/home/your_username/voice-typing-linux` with the actual path to your checkout.
You can get it with `pwd` inside the project directory.
...
If librime is installed, the IBus engine embeds a Rime session for Chinese
input. Press **Shift** to toggle between EN/voice mode and Chinese mode.
Candidates appear inline in the preedit.

Rime is optional — if librime is not found the engine starts normally with
Chinese mode disabled.

### Changing the input schema

By default the engine uses `double_pinyin_mspy` (微软双拼). To switch to
another schema, set `RIME_SCHEMA` before launching:
```bash
# 自然码双拼
export RIME_SCHEMA=double_pinyin
# 全拼
export RIME_SCHEMA=luna_pinyin
# 五笔
export RIME_SCHEMA=wubi86
./voice --streaming
```

The schema must already be installed in your Rime user directory
(`~/.config/ibus/rime` by default). If you use fcitx-rime or a custom
location, set `RIME_USER_DIR` as well:
```bash
...
|----------|---------|-------------|
| `RIME_SCHEMA` | `double_pinyin_mspy` | Schema ID (must match a `.schema.yaml` in your Rime dir) |
| `RIME_USER_DIR` | `~/.config/ibus/rime` | Rime user data directory |
| `RIME_SHARED_DIR` | `/usr/share/rime-data` | Rime shared data directory (rarely needed) |

### Known limitation

The `;` key is not forwarded to Rime. Users of schemas that assign `;` as
an input key (e.g. `double_pinyin_mspy` uses it for `ing`) should switch to
a schema that does not require it, such as `double_pinyin` (自然码).
...
| zipformer-zh | ~488 MB | Fast | Chinese/English bilingual streaming |
...
This project is based on [voice-typing-linux](https://github.com/GitJuhb/voice-typing-linux)
by GitJuhb, originally licensed under the MIT License.

Modifications and additions (Rime IME integration, zipformer-zh, Fedora launcher)
Copyright (c) 2025 Mathompo. Licensed under the GNU General Public License v3.0 —
see [LICENSE](LICENSE).
...
- [Rime](https://rime.im) — input method engine for Chinese
