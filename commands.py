...
PATTERN_COMMANDS = [
    ...
    (
        r"^search for (.+)$",
        {"intent": "search", "action": "web_search", "param": "query"},
    ),
...
class CommandDetector:
    ...
    def __init__(
        self, custom_commands_path: Optional[str] = None, enabled: bool = True
    ):
            ...
            if "+" in key_part:
                ...
                for mod in modifiers:
                    if mod in YDOTOOL_KEYCODES:
                        subprocess.run(
                            ["ydotool", "key", f"{YDOTOOL_KEYCODES[mod]}:1"], env=env
                        )
                ...
                for mod in reversed(modifiers):
                    if mod in YDOTOOL_KEYCODES:
                        subprocess.run(
                            ["ydotool", "key", f"{YDOTOOL_KEYCODES[mod]}:0"], env=env
                        )
            else:
                # Single key
                key = key_part
        ...
        if action == "focus":
            ...
            if app:
                ...
                if result.returncode != 0 and self._get_display_server() == "x11":
                    ...
                    subprocess.run(
                        ["xdotool", "search", "--name", app, "windowactivate", "--sync"]
                    )
                return True
